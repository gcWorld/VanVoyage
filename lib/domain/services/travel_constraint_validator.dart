...
class TravelConstraintValidator {
  ...
  List<ConstraintViolation> validate(TripPreferences preferences) {
    ...
    violations
        .addAll(_validateDailyDistance(preferences.maxDailyDrivingDistance));
  ...
  List<ConstraintViolation> _validateDailyDistance(int distance) {
    ...
    if (distance < constraints.minDailyDistance) {
      violations.add(ConstraintViolation.error(
        ...
        message:
            'Daily distance of $distance km is too low. Minimum is ${constraints.minDailyDistance} km.',
      ...
      violations.add(ConstraintViolation.error(
        field: 'maxDailyDrivingDistance',
        message:
            'Daily distance of $distance km exceeds safe limit. Maximum is ${constraints.maxDailyDistance} km.',
        currentValue: distance,
      ...
      violations.add(ConstraintViolation.warning(
        field: 'maxDailyDrivingDistance',
        message:
            'Daily distance of $distance km is below recommended range. Consider ${constraints.recommendedMinDistance}-${constraints.recommendedMaxDistance} km per day.',
        currentValue: distance,
        expectedValue:
            '${constraints.recommendedMinDistance}-${constraints.recommendedMaxDistance} km',
      ));
    } else if (distance > constraints.recommendedMaxDistance) {
      violations.add(ConstraintViolation.warning(
        field: 'maxDailyDrivingDistance',
        message:
            'Daily distance of $distance km is above recommended range. Consider ${constraints.recommendedMinDistance}-${constraints.recommendedMaxDistance} km per day for a comfortable trip.',
        currentValue: distance,
        expectedValue:
            '${constraints.recommendedMinDistance}-${constraints.recommendedMaxDistance} km',
      ));
  ...
  List<ConstraintViolation> _validateDailyTime(int timeInMinutes) {
    ...
    if (timeInMinutes < constraints.minDailyTime) {
      violations.add(ConstraintViolation.error(
        ...
        message:
            'Daily driving time of $hours hours is too short. Minimum is ${(constraints.minDailyTime / 60).toStringAsFixed(1)} hours.',
      ...
      violations.add(ConstraintViolation.error(
        field: 'maxDailyDrivingTime',
        message:
            'Daily driving time of $hours hours exceeds safe limit. Maximum is ${(constraints.maxDailyTime / 60).toStringAsFixed(1)} hours to avoid fatigue.',
        currentValue: timeInMinutes,
      ...
      violations.add(ConstraintViolation.warning(
        field: 'maxDailyDrivingTime',
        message:
            'Daily driving time of $hours hours is below recommended range. Consider ${(constraints.recommendedMinTime / 60).toStringAsFixed(1)}-${(constraints.recommendedMaxTime / 60).toStringAsFixed(1)} hours per day.',
        currentValue: timeInMinutes,
        expectedValue:
            '${constraints.recommendedMinTime}-${constraints.recommendedMaxTime} minutes',
      ));
    } else if (timeInMinutes > constraints.recommendedMaxTime) {
      violations.add(ConstraintViolation.warning(
        field: 'maxDailyDrivingTime',
        message:
            'Daily driving time of $hours hours is above recommended range. Consider ${(constraints.recommendedMinTime / 60).toStringAsFixed(1)}-${(constraints.recommendedMaxTime / 60).toStringAsFixed(1)} hours per day to avoid driver fatigue.',
        currentValue: timeInMinutes,
        expectedValue:
            '${constraints.recommendedMinTime}-${constraints.recommendedMaxTime} minutes',
      ));
  ...
  List<ConstraintViolation> _validateSpeed(int speed) {
    ...
    if (speed < constraints.minSpeed) {
      violations.add(ConstraintViolation.warning(
        ...
        message:
            'Driving speed of $speed km/h is very slow. This may result in unrealistic travel time estimates.',
        currentValue: speed,
      ...
      violations.add(ConstraintViolation.warning(
        field: 'preferredDrivingSpeed',
        message:
            'Driving speed of $speed km/h is very high. Consider local speed limits and road conditions.',
        currentValue: speed,
  ...
  List<ConstraintViolation> _validateRestInterval(int intervalInMinutes) {
    ...
    if (intervalInMinutes < constraints.minRestInterval) {
      violations.add(ConstraintViolation.warning(
        ...
        message:
            'Rest interval of $hours hours is too frequent. This may add excessive travel time.',
      ...
      violations.add(ConstraintViolation.error(
        field: 'restStopInterval',
        message:
            'Rest interval of $hours hours is too long. Take breaks every ${(constraints.maxRestInterval / 60).toStringAsFixed(1)} hours or less to stay alert.',
        currentValue: intervalInMinutes,
      ...
      violations.add(ConstraintViolation.warning(
        field: 'restStopInterval',
        message:
            'Rest interval of $hours hours is below recommended. Consider ${(constraints.recommendedMinRestInterval / 60).toStringAsFixed(1)}-${(constraints.recommendedMaxRestInterval / 60).toStringAsFixed(1)} hours between breaks.',
        currentValue: intervalInMinutes,
        expectedValue:
            '${constraints.recommendedMinRestInterval}-${constraints.recommendedMaxRestInterval} minutes',
      ));
    } else if (intervalInMinutes > constraints.recommendedMaxRestInterval) {
      violations.add(ConstraintViolation.warning(
        field: 'restStopInterval',
        message:
            'Rest interval of $hours hours is above recommended. Consider taking breaks every ${(constraints.recommendedMinRestInterval / 60).toStringAsFixed(1)}-${(constraints.recommendedMaxRestInterval / 60).toStringAsFixed(1)} hours.',
        currentValue: intervalInMinutes,
        expectedValue:
            '${constraints.recommendedMinRestInterval}-${constraints.recommendedMaxRestInterval} minutes',
      ));
  ...
  List<ConstraintViolation> _validateConsistency(TripPreferences preferences) {
    ...
    final maxDistanceByTime = (preferences.maxDailyDrivingTime / 60) *
        preferences.preferredDrivingSpeed;

    if (preferences.maxDailyDrivingDistance > maxDistanceByTime * 1.2) {
      violations.add(ConstraintViolation.warning(
        field: 'consistency',
        message:
            'Your max daily distance (${preferences.maxDailyDrivingDistance} km) may be unreachable with ${(preferences.maxDailyDrivingTime / 60).toStringAsFixed(1)} hours of driving at ${preferences.preferredDrivingSpeed} km/h. Maximum achievable is approximately ${maxDistanceByTime.toInt()} km.',
    ...
    if (preferences.includeRestStops && preferences.restStopInterval != null) {
      if (preferences.restStopInterval! >= preferences.maxDailyDrivingTime) {
        violations.add(ConstraintViolation.warning(
          field: 'restStopInterval',
          message:
              'Rest interval (${(preferences.restStopInterval! / 60).toStringAsFixed(1)} hrs) should be less than daily driving time (${(preferences.maxDailyDrivingTime / 60).toStringAsFixed(1)} hrs).',
