...
class _TripPreferencesFormState extends State<TripPreferencesForm> {
  ...
  void initState() {
    ...
    _maxDailyDistance = (widget.preferences?.maxDailyDrivingDistance ?? 300)
        .toDouble()
        .clamp(100.0, 800.0);
    _maxDailyTime = (widget.preferences?.maxDailyDrivingTime ?? 240)
        .toDouble()
        .clamp(60.0, 600.0);
    _preferredSpeed = (widget.preferences?.preferredDrivingSpeed ?? 80)
        .toDouble()
        .clamp(50.0, 120.0);
    _includeRestStops = widget.preferences?.includeRestStops ?? true;
    _restStopInterval = (widget.preferences?.restStopInterval ?? 120)
        .toDouble()
        .clamp(60.0, 300.0);
  ...
  void _initializePhaseConstraints() {
    ...
    if (hasPhaseConstraints) {
      ...
      _transitMaxDailyDistance =
          prefs.transitMaxDailyDrivingDistance?.toDouble().clamp(100.0, 800.0);
      _transitMaxDailyTime =
          prefs.transitMaxDailyDrivingTime?.toDouble().clamp(60.0, 600.0);
      _vacationMaxDailyDistance =
          prefs.vacationMaxDailyDrivingDistance?.toDouble().clamp(100.0, 800.0);
      _vacationMaxDailyTime =
          prefs.vacationMaxDailyDrivingTime?.toDouble().clamp(60.0, 600.0);
    }
  }
      ...
      child: Column(
        ...
        children: [
          ...
          if (_violations.isNotEmpty) ...[
            ...
            const SizedBox(height: 16),
            ..._violations
                .map((violation) => _buildViolationCard(context, violation)),
            const SizedBox(height: 24),
          ],
          ...
          Card(
            child: SwitchListTile(
              ...
              subtitle:
                  const Text('Different limits for transit vs vacation phases'),
  ...
  Widget _buildViolationCard(
      BuildContext context, ConstraintViolation violation) {
